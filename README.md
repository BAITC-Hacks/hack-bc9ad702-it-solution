...
Сообщение «Сервер не рассчитал сценарий» обычно означает, что на порту `8081` осталась старая версия сервера. Остановите предыдущий процесс через `Ctrl+C`, снова запустите `run.bat` и обновите страницу сочетанием `Ctrl+F5`.
