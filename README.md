...
![Главная страница AstanaSIM AI с выбранными инициативами](<./Снимок экрана_23-9-2026_17209_localhost.jpeg>)

### Итоговый дашборд, карта и AI-анализ

![Итоговый экран AstanaSIM AI с картой районов и сравнением команд](<./Снимок экрана_23-9-2026_172026_localhost.jpeg>)
...
3. При первом запуске скрипт создаст `.venv`. Если `OPENAI_API_KEY` ещё не задан, он предложит вставить ключ. При пустом вводе приложение всё равно запустится, а объяснение результата сформирует детерминированный fallback. В `.env` не записывается шаблонный или выдуманный ключ.
4. Откройте в браузере адрес, показанный в терминале. По умолчанию это [http://localhost:8081](http://localhost:8081). Если порт занят, `run.bat` автоматически выберет следующий свободный порт в диапазоне 8081–8090.
...
Если основной порт занят старой версией сервера, `run.bat` выведет предупреждение и запустит актуальную версию на следующем свободном порту. Откройте именно адрес из терминала. При сообщении «Сервер не рассчитал сценарий» обновите страницу сочетанием `Ctrl+F5` и проверьте, что адрес и порт совпадают с указанными в окне `run.bat`.
