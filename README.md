...
data/model-data.json
 ...
 app.js ───────────────► index.html + styles.css
   │                          │
   │ коды мер и районы         │ выбор, карта, localStorage
   ▼                          ▼
 server.py ───────────► model_engine.py ◄── tests/model_test.py
   │                          │
   │ проверенный результат     │ читает веса, эффекты и правила
   ├────────► OpenAI API       │
   │              │            │
   └── fallback ◄─┘            │
              │                │
...
- `server.py` раздаёт приложение, публикует `/api/model-summary`, `/api/simulate` и `/api/analyze`;
...
| --- | --- |
| `data/model-data.json` | Единственный активный и достаточный источник модели в репозитории: районы, веса, инициативы, эффекты, синергии и ограничения. |
