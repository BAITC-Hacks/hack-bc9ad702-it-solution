...
2. Дважды нажмите `run.command`. При первом запуске macOS может потребовать разрешить открытие файла в разделе **Системные настройки → Конфиденциальность и безопасность**.
3. Если файл не запускается из-за отсутствия права выполнения, один раз выполните в Terminal:
...
Откройте в браузере адрес, показанный в терминале. По умолчанию это [http://localhost:8081](http://localhost:8081). Если порт занят, скрипт автоматически выберет следующий свободный порт в диапазоне 8081–8090.
